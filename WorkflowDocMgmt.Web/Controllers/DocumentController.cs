...
public class DocumentController : Controller
{
    ...
    // Update Status (Approve/Reject & Move Next Stage / Reassign)
    public IActionResult UpdateStatus(int id, string status, int? nextStageId = null, bool reassign = false)
    {
        string currentUser = User.Identity.Name ?? "Anonymous";

        using (SqlConnection con = new SqlConnection(_config.GetConnectionString("DefaultConnection")))
        {
            SqlCommand cmd = new SqlCommand("sp_UpdateDocumentStatus", con); // Updated SP name
            cmd.CommandType = CommandType.StoredProcedure;
            ...
            cmd.Parameters.AddWithValue("@Reassign", reassign); // for reassigning document to first stage
